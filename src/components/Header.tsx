        ...
        <header className="fixed top-0 w-full left-1/2 -translate-x-1/2 bg-background/95 backdrop-blur z-50 border-b border-border px-4 md:px-8 py-3 flex items-center justify-between shadow-sm">
            <div className="max-w-7xl mx-auto w-full flex items-center justify-between">
                ...
                <nav className="hidden md:flex items-center gap-6">
                    <Link href="/" className="text-sm font-medium hover:text-primary transition-colors">{t('common.portfolio')}</Link>
                    <Link href="/analysis" className="text-sm font-medium hover:text-primary transition-colors">{t('common.analysis')}</Link>
                    <Link href="/lab" className="text-sm font-medium hover:text-primary transition-colors flex items-center gap-1.5 px-3 py-1 bg-primary/10 rounded-full text-primary border border-primary/20">
                        <Beaker className="w-4 h-4" />
                        {t('lab.title')}
                    </Link>
                    <Link href="/screener" className="text-sm font-medium hover:text-primary transition-colors">{t('common.screener')}</Link>
                    <Link href="/chat" className="text-sm font-medium hover:text-primary transition-colors">{t('common.aiAssistant')}</Link>
                </nav>
                                ...
                                <div className="absolute right-0 top-full mt-2 w-64 bg-card border border-border rounded-2xl shadow-2xl flex flex-col p-2 z-50 animate-in fade-in zoom-in duration-200">
                                    {user && (
                                        <div className="px-3 py-3 mb-2 border-b border-border/50">
                                            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-1">{t('header.loggedInAs')}</p>
                                    ...
                                    <Link
                                        href="/lab"
                                        onClick={() => setIsNavMenuOpen(false)}
                                        className="flex items-center gap-3 px-3 py-2.5 text-sm font-medium text-foreground hover:bg-muted rounded-lg transition-colors border-l-2 border-primary"
                                    >
